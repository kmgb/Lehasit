...
struct trace_t {
	Vector start;
	Vector end;
	unsigned char plane[20]; //cplane_t
	float fraction;
	int contents;
	...
	unsigned char surface[8]; //csurface_t
	int hitgroup;
	short physics_bone;
	CBaseEntity* pEntity;
	int hitbox;
...
struct Ray_t {
	...
	VectorAligned start_offset;    // Add this to m_Start to get the actual ray start
	...
	void init(const Vector& vecStart, const Vector& vecEnd) {
		...
		extents.x = extents.y = extents.z = 0.f;
		is_ray = true;

		start_offset.x = start_offset.y = start_offset.z = 0.f;
